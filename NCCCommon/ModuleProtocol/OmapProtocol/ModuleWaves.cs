    ...
    public class ModuleWaves
    {
        ...
        public static ModuleWaves Parse(DspMessage msg, TimeSpan? timeOffset)
        {
            ...
            for (int ch = 0; ch < 8; ch++)
            {
                ...
                i = WaveData.ReadHeader(ref data, i, buff);
                i = WaveData.ReadBody(ref data, i, buff);
                if (timeOffset != null)
                {
                    var newTime = data.DateTime.ToLocalDateTime() + timeOffset.Value;
                    data.DateTime = new UtcAndMiliseconds(newTime);
                }
                waves[ch] = data;
            }
